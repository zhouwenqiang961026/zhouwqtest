vds.config({
    ...
    vds.mock.init("../manifest.json").then(function(mock){
        ...
        window._testDV = true;
        for(var i=0;i<40;i++){
            mock.get("ListToString").then(function(functionMock){
                functionMock.mockInput(0,"entity");
                functionMock.mockInput(1,"sum");
                functionMock.mockInput(2,"_$_");
                if(window._testDV){
                ...
                for(var i=0,l=args.length;i<l;i++){
                    html += '<span>入参';
                    html += i;
                    html += '：</span><span style="display:inline-block;width:10%;overflow:auto;">';
                    html += args[i];
                    html += '</span>';
                }
                try{
                    ...
                    html += '<span>结果：</span><span style="display:inline-block;width:10%;overflow:auto;">';
                    html += result;
                    html += '</span>';
                    div.innerHTML = html;
                    ...
                    div.style.color = "red";
                    div.style.border = "2px solid #f5f5f5";
                    html += '<span>异常原因：</span><span style="display:inline-block;width:10%;overflow:auto;">';
                    html += e.message;
                    html += '</span>';
                    div.innerHTML = html;
                    document.body.appendChild(div);
