vds.config({
    debug:true,
    import:["vds.mock.*","vds.math.*"]
}).ready(function(){
    vds.mock.init("../manifest.json").then(function(mock){
        var dv = true;
        for(var i=0;i<40;i++){
            mock.get("PadLeft").then(function(functionMock){
                if(dv){
                    functionMock.mockInput(0,parseInt(Math.random() * 10000));
                    functionMock.mockInput(1,"18");
                    functionMock.mockInput(2,"6");
                    dv = false;
                }
